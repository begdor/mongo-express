...
exports.calSummary = function(items){
  ...
  for (var i in items) {
    ...
    if (items[i].status === 'executed' || items[i].status === 'executedArchived') {
      ...
      //calculate Days To Executed(DTE)
      let createDate = new Date(items[i].createdDate), exeDate = new Date(items[i].executionDate);
      sumDTE = sumDTE + Math.round((exeDate.getTime() - createDate.getTime())/(24*60*60*1000));
  ...
  if (executed.total !== 0) {
    ...
    executed.avgDTE = (sumDTE/executed.total).toFixed(1);
  }
  if (open.total !== 0) {
