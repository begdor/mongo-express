...
var convert = require('../convertToCSV');
...
var routes = function (config) {
  ...
  exp.viewCollection = function (req, res) {
    ...
    if (key && value) {
      ...
      if (type.toUpperCase() === 'O') {
        ...
        value = ObjectId(req.query.value);
      }
      if (type.toUpperCase() === 'D') {
        value = new Date(Date.parse(req.query.value));
      }
    ...
    // shadowbid: do the summary calcualtions here for all results
    var sumData = {}, openPercent = 0, exePercent = 0;
    ...
    req.collection.find().toArray(function(err,items){
      req.collection.stats(function (err, stats) {
        if(err){
          ...
          req.collection.count(function (err, countAll) {
            sumData = summary.calSummary(items);
            openPercent = (sumData.open.total/countAll * 100).toFixed(2);
            exePercent = (sumData.executed.total/countAll * 100).toFixed(2);
                ...
                req.collection.count(query, null, function (err, count) {
                  ...
                  for (var i in items) {
                    ...
                    for (let prop in items[i]) {
                      //shadowbid: to remove product and add its property to the column
                      if (prop === 'tax' || prop === 'shippingCost' || prop ==='__v') {
                        delete items[i][prop];
                      }

                      if(prop === 'product') {
                        var product = items[i][prop];
                        delete items[i][prop];
                        for (var j in product) {
                          if (j === 'highPrice' || j === 'lowPrice' || j === 'pricingDuration' || j === 'currentPrice'
                          || j === 'amazonUrl' || j === 'listPrice') {
                            items[i][j] = product[j];
                          }
                        }
                      }
                      else if (utils.roughSizeOfObject(items[i][prop]) > config.options.maxPropSize) {
                        items[i][prop] = {
                          attribu: prop,
                    ...
                    //TODO: add reasonable keys in the filter array
                    items[i] = bson.toString(items[i]);
                  }
                  /*
                  // shadowbid: do the summary calcualtions here if display dynamically
                  sumData = summary.calSummary(docs);
                  openPercent = (sumData.open.total/count * 100).toFixed(2);
                  exePercent = (sumData.executed.total/count * 100).toFixed(2);
                  */
                  // Generate an array of columns used by all documents visible on this page
                  columns = _.uniq(_.flatten(columns));
                  for (let i in columns) {
                    let field = columns[i];
                    if(field !== 'endDate' && field !== 'info' && field !== 'amazonUrl' && field !== 'currentPrice' && field !== 'highPrice'
                    && field !== 'lowPrice' && field !== 'pricingDuration' && field !== 'listPrice' && field !== 'endDate') {
                      filterCol.push(field);
                    }
                  }
                  var ctx = {
  ...
  //shadowbid: add a export csv function
  exp.exportCSV = function (req,res) {
    var query = req.query;
    req.collection.find(query).toArray(function (err, items){

      //extract infomation from product
      for (var i in items){
        for (let prop in items[i]) {
          //shadowbid: to remove product and add its property to the column
          if(prop === 'product') {
      ...
      var content = convert.convertToCSV(items);
      res.write(content);
      res.end();
    });
  };
  ...
  //shadowbid: to retrieve all distinct values to corresponding  key
  exp.generateOptions = function (req,res) {
    var options = {};
    var key = req.query.key;
    req.collection.find().toArray(function (err, items) {
      for(var i in items) {
        for (let prop in items[i]) {
          //extract fields in product object
          if(prop === 'product') {
            var product = items[i][prop];
            delete items[i][prop];
            //TODO: query the fields in the product object
            /*
            for (var j in product) {
              if (j === 'highPrice' || j === 'lowPrice' || j === 'pricingDuration' || j === 'currentPrice'
              || j === 'amazonUrl' || j === 'listPrice') {
        ...
        if (key !== '' && !(items[i][key] in options)) {
          if ((typeof items[i][key]) === 'object'){
            options[items[i][key].toString()] = items[i][key].toString();
          }
          else options[items[i][key]] = items[i][key];
        }
      }
      res.json(options);
    });
  ...
  exp.generateSums = function (req,res) {
    req.collection.find().toArray(function(err,items){
      res.json(items);
    });
  }

  exp.renderLineChart = function (req,res) {
