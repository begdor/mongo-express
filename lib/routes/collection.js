      ...
      req.collection.stats(function (err, stats) {
        req.collection.count(query, null, function (err, count) {
          ...
          for (var i in items) {
            ...
            for (let prop in items[i]) {
              ...
              if(prop === 'product') {
                ...
                for (var j in product) {
                  ...
                  || j === 'amazonUrl' || j === 'listPrice' || j === 'brand' || j === 'name' || j === 'parentSku'
                  || j === 'category' || j === 'modelNumber' || j === 'isLoadPricingData' || j === 'isEligibleForSuperSaverShipping'
                  || j === 'merchant' || j === 'isEligibleForPrime') {
                    items[i][j] = product[j];
                  }
                }
              }
            ...
            if (key !== '' && currentValues[items[i][key]] === null) {
              if ((typeof items[i][key]) === 'object'){
                currentValues[items[i][key].toString()] = items[i][key].toString();
              }
              else currentValues[items[i][key]] = items[i][key];
            }
            docs[i] = items[i];
  ...
  exp.generateOptions = function (req,res) {
    ...
    req.collection.find().toArray(function (err, items) {
      for(var i in items) {
        ...
        if (key !== '' && options[items[i][key]] == null) {
          if ((typeof items[i][key]) == 'object'){
